...
<div class="pemesanan-form">
    <div class="card">
        ...
        <div class="card-body">
            <form action="/godmode/pemesanan/store" method="POST" class="needs-validation" novalidate>
                <?= csrf_field() ?>
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-select <?= session('errors.pelanggan_id') ? 'is-invalid' : '' ?>"
                                ...
                                name="pelanggan_id"
                                required>
                                <option value="" selected disabled>Pilih Pelanggan</option>                                <?php if (isset($pelanggans) && count($pelanggans) > 0): ?>
                                    <?php foreach ($pelanggans as $pelanggan): ?>
                                        <option value="<?= $pelanggan['id'] ?>" <?= old('pelanggan_id') == $pelanggan['id'] ? 'selected' : '' ?>>
                                            <?= esc($pelanggan['username'] ?? '-') ?> (<?= esc($pelanggan['email'] ?? '-') ?>)
                                        </option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                      ...
                      <div class="col-md-6">                        <div class="form-group">                            <label for="tanggal_pemesanan" class="form-label">
                    ...
                    <div class="col-md-6">                        <div class="form-group">                            <label for="status_pemesanan" class="form-label">
                            ...
                            <select class="form-select <?= session('errors.status_pemesanan') ? 'is-invalid' : '' ?>"
                                ...
                                required>
                                <option value="menunggu" <?= old('status_pemesanan') === 'menunggu' ? 'selected' : '' ?>>Menunggu</option>
